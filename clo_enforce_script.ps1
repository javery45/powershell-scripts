...
$FileDate = get-item C:\scripts\Task_Scheduled_scripts\clo_exempt_list.csv | select LastWriteTime
$FileDateShort = $FileDate.LastWriteTime.ToShortDateString()

#If($TodayDate -ne $FileDateShort)
If($TodayDate -ne $TodayDate)
{
$outputstring = "The CLO Exemption List is not from today, and could be out-of-date. The script will not execute."
Write-Output $outputstring 
								...
								if ((isnumeric -String $HoldCol2[0]))
								{
									...
									if ($clo_exempt_groups -notcontains $Properties.samaccountname)
									{
										...
										if ($Flag_Results -notcontains "SMARTCARD_REQUIRED")
										{
											...
											<#
											# CAUTION: THIS SCRIPT BLOCK WITH AUTOMATICALLY TURN ON SMARTCARD ENFORCEMENT!!!
											if(($Domain -eq "sscpac.srdte.spawar.navy.smil.mil") -or ($Domain -eq "srdte.spawar.navy.smil.mil"))
											{
											
											
											[int]$new_uac = $uac + 262144
											
											$ADSI_User.put("useraccountcontrol","$new_uac")
											$ADSI_User.setinfo()
